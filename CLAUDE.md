...
- **M5** — NAT & relays (DCUtR / AutoNAT / Relay v2, capped opt-in relaying). *(Also the home for **connection robustness**: reconnection / retry, address persistence & refresh, reusing an already-open connection, and revisiting the 60 s idle timeout. M2b is best-effort with none of these — see "Known limitations" in `docs/net.md`; the M3 TUI only *displays* connection state honestly, it does not heal it.)* **Also the home for unlock-at-startup for an always-on node** (surfaced by the M4 real-internet test): a headless VPS bootstrap/relay cannot publish or serve until its keystore is unlocked, which today means a human at startup. Any auto-unlock **weakens at-rest protection** — the passphrase must be reachable by the machine, so the trade-off has to be stated honestly, not hidden (options to weigh at scoping: an OS-held credential such as a systemd credential / TPM, an interactive unlock at boot, or a **bootstrap-only mode that needs no messaging identity at all**, since a DHT entry point arguably needs only a transport keypair — that last one is a design change, not a config flag). Partly a packaging concern (see the packaging bullet in Long-term vision).
